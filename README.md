...
<p align="center">
  <img src="logo.png" alt="dock.ai logo" width="400">
</p>

## What it is and why

dock.ai builds a Docker image from an existing base image and adds AI-assisted development tools for the normal user inside the container.

The goal is to keep the base image separate from the AI tooling. You start with a base image that already contains your main development environment, and dock.ai creates a derived image that keeps that environment while adding tools such as Bun, pnpm, context-mode, context7-mcp, and RTK.
...
The base image must contain the user you want to use as the main container user. dock.ai uses `dev` as the default user.
...
```bash
./build_ai_image.sh my-base:latest my-base-ai:latest developer .profile /path/to/dock.ai
```
